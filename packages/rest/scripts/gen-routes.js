...
function slugToDocsURL(slug) {
  const [docs, category, ...pageName] = slug.replace(/^\//, '').toLowerCase().split('_');
    ...
    if ((tempMatch = line.match(regexEndpointHeader)) || line.startsWith('## ')) {
      if (currentEndpoint) {
        ...
        const tsTypeNameBase =
          ...
          `Discord.REST${capitalize(method.toLowerCase())}API` +
            endpoint
              .slice(1)
              .split('/')
              .map(x => x.replace(/\${(.*)}/, (_, x) => `:${x.replace(/Id$/, '')}`))
              .join('/')
              .replace(
        ...
        const routeValueJS =
          params.length === 0
            ? `"${endpoint}"`
            : `(${params.map(x => `${x}: string`).join(', ')}) => \`${endpoint}\``;
        ...
        const queryValueJS = mdContent.includes('###### Query String Params')
          ? `query: type<${overrides.query[camelCasedName] ?? `${tsTypeNameBase}Query`}>(),`
          : '';

        const bodyValueJS =
          ['POST', 'PUT', 'PATCH'].includes(currentEndpoint[2]) &&
          !overrides.noBody.includes(camelCasedName)
            ? `body: type<${overrides.body[camelCasedName] ?? `${tsTypeNameBase}JSONBody`}>(),`
            : '';

        const formValueJS = overrides.form[camelCasedName]
          ? `form: type<${overrides.form[camelCasedName]}>(),`
          : '';

        const hasNoContent = mdContent.includes('204 No Content');
        const resultValueJS = `result: type<${
...
const finalFile = `// This file is generated by scripts/gen-routes.js
...
writeFileSync('./src/routes.generated.ts', finalFile);
